...
def model(dbt, session):
    ...
    fil_cov = covariates[
        (covariates["ACTIVE_DRIVER"] == 1) & (covariates["ACTIVE_CONSTRUCTOR"] == 1)
    ]
    ...
    fil_cov["TOTAL_PIT_STOPS_PER_RACE"] = le.fit_transform(
        fil_cov["TOTAL_PIT_STOPS_PER_RACE"]
    )
    ...
    encoded_data["POSITION_LABEL"] = encoded_data["POSITION"].apply(
        lambda x: position_index(x)
    )
